...
def eq_eval(x, y, z, coeffs):
    a, b, c = coeffs
    return b - c * z - ((x - a) ** 2 + y**2) * ((x + a) ** 2 + y**2)
...
if __name__ == "__main__":
    ...
    dxy_length = 1.0 / 16  # xy平面移動時ステップ移動量。
    dz_length = 1.0 / 16  # z上方向移動時ステップ移動量。
    ...
    z_stop = 1.99  # 高さzがz_stopに達したら処理終了。

    a = create_surface_ply(eq_eval, coeffs, stop_k, point_regist_k, valid_thr)

    pinit = np.array([1.5, 0, 0])  # 初期位置。面上の点が望ましい。
    dxy_dir = np.array([0, 1, 0])  # 初期位置から次の点に移動するxy平面上の方向。
    ...
    a.write_ply("cassini_oval_2.ply")
