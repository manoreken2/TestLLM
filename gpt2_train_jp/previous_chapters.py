...
def create_dataloader_v1(txt, tokenizer, cfg, shuffle=True, drop_last=True, num_workers=0):
    dataset = GPTDatasetV1(txt, tokenizer,
                            cfg['context_length'],
...
def generate_and_print_sample2(model, tokenizer, device, start_context, max_new_tokens, temperature=1.4):
    model.eval() # ドロップアウト率を0にする。
    token_ids = generate(
        model=model,
...
def plot_loss_perplexities(name, epochs_seen, tokens_seen, train_losses, val_losses):
    ...
    val_perplex = [math.exp(x) for x in val_losses]

    ax1.plot(epochs_seen, train_losses, label="Training loss")
    ax1.plot(epochs_seen, val_losses, linestyle="-.", label="Validation loss")
    ax1.plot(epochs_seen, train_perplex, label="Training perplexity")
    ax1.plot(epochs_seen, val_perplex, linestyle="-.", label="Validation perplexity")
    ax1.set_xlabel("Epochs")
    ax1.set_ylabel("Loss and Perplexity")
    ax1.set_yscale('log')
    ax1.legend(loc="upper right")
