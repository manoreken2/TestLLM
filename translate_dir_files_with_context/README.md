...
1. Enable Windows developer mode to use symlinks

    Refer this document https://docs.microsoft.com/en-us/windows/apps/get-started/enable-your-device-for-development
...
2. Download and install Miniforge.
    ...
    The program is tested with Miniforge3-25.3.1-0. https://github.com/conda-forge/miniforge/releases/download/25.3.1-0/Miniforge3-25.3.1-0-Windows-x86_64.exe

4. Download this TestLLM repository as zip and extract to C:\work as C:\work\TestLLM

5. Download GLM-5.2 GGUF parameter file from Huggif Face

    Open Miniforge3 prompt and type the following commands to create hf_download conda environment.
...
5. Download GLM 5.2 GGUF parameter file.
    ...
    Visit https://huggingface.co/ and sign in, Create user access token, it is free of charge. Token is some text string beginning with hf.

    On Miniforge3 prompt, type the following commands. **Replace hf_YOUR_USER_ACCESS_TOKEN_HERE with your token**

    ```
    conda activate hf_download
    hf download unsloth/GLM-5.2-GGUF --local-dir unsloth/GLM-5.2-GGUF --include "*UD-Q6_K_XL*" --token hf_YOUR_USER_ACCESS_TOKEN_HERE --max-workers 1
    ...
    Now GLM-5.2 parameter file is on C:\work\hf

5. Test run GLM 5.2 model on Llama
...
6. Start Llama server on your PC.
    ...
    Now llama-server is running and waiting incoming request. llama-server quits when CMD window is closed so keep CMD window open while using.
